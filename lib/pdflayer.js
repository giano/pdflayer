...
class Pdflayer {
     ...
     * 
     *    options = {
     *        api_url: 'http://api.pdflayer.com/api/convert'     // PdfLayer base api url. Defaults to http://api.pdflayer.com/api/convert
    ...
    constructor(options = {}) {
        ...
        this.config = _.pick(_.defaults({}, options, {
            ...
            method: 'POST'
        }), 'api_url', 'method', 'access_key', 'rate_limit');
        this.queue = rateLimit.createQueue({ interval: this.config.rate_limit ? (60000 / this.config.rate_limit) : 100 });
        this.defaultRequestConfig = getConfigKeys(_.defaults({}, options, DEFAULT_REQUEST_CONFIG));
        this.wrapper = new RequestWrapper(this.config);
    }
    ...
    generate(htmlOrUrl = '', options = {}) {
        return new Promise((resolve, reject) => {
            ...
            let fnToCall = () => {
                ...
                return this.wrapper.request(options).then(resolve).catch(reject);
            };
            this.queue.add(fnToCall);
        });
    }

    /**
     * Generate a valid GET Pdflayer url. Does not count against rate limiting requests. SYNC function.
     * @param {string} url                                  // The url to document
     * @param {Object} [options={}]                         // Options for this request
     * @memberof Pdflayer
     * @return {String}  Returns the Pdflayer complete url.
     * 
     */
    getUrl(url = '', options = {}) {
        url = _.trim(url);
        if (url && validUrl.isUri(url)) {
            ...
            throw new Error('You should pass an URL to the document');
        }
        return this.wrapper.generateUrl(options);
    }
};
