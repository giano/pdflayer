...
class RequestWrapper {
    ...
    request(options = {}) {
        return new Promise((resolve, reject) => {
            ...
            requestOptions.qs = { access_key: options.access_key };

            if (requestOptions.method.toUpperCase() === 'GET') {
                requestOptions.qs = _.extend(options, requestOptions.qs);
            } else {
                requestOptions.form = options;
                delete requestOptions.form.access_key;
            }

            let reReadable = new Rereadable();
